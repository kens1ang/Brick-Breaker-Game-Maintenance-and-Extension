...
1. **Initialization** (`Model` package, `GameModel` class, `setballpos` method)
    ...
    - Randomise game ball position cause ball spawn inside the blocks as higher levels the number of blocks increase. 
    ...
    - `setballpos` method calculate the height of lowest block and set the position of game ball at the center between the lowest block and paddle. 
...
1. **Bomb blocks only final level** (`Model` package, `GameModel` class, `initboardmodel` method)
    - Introduced a new game level where all the blocks are bomb blocks, create a challenging final level. 
    - When the level is 18, all blocks are now bomb blocks, the method `determineBlockType` has been adjusted to handle this scenario.
...
1. **SoundManager class**(`brickGame` package, `SoundManager` class)
    ...
    - Add sound effects on different action events for example, Collect bonus, Blocks hit, Game Over etc., to enhance gaming experience. Background music contributes to overall atmosphere of the game.  
2. **Playsounds method in necessary methods** (`Model` package, `GameModel` class)
...
1. **Space bar key** (`Controller` package, `GameController` class, `handle`, `pauseresumeGame` methods)
    - `handle` method to receive users' input to pause or resume the game. (improve gaming experience)
...
- Design interface might be considered outside the scope of this project
  
### Multi-ball
- Limited development time for this complex features and this features require lot of development effort as functionality is complex considerate ball physics, collisions and user interaction
- Multiple features could introduce new challenges in testing, debugging and refactoring
...
### BlockSerializable
- This class moved to `Model.GameElements` package
- Added a field `colorIndex` to store the index of the color in the colors array.
...
### Load game issue:
1. After load the saved game, after all blocks are destroyed the level **doesn't proceed to next level**.
...
2. **The colors of blocks is not saved**
    - Added a field `colorIndex` to the `BlockSerializable` class to store the index of the color in the colors array.
    - Updated the constructor to include the `colorIndex` parameter.
...
1. **Shared data in multithreaded environment**:
    - When multiple threads are involved, it's crucial to synchronize access to shared resources. There are bugs and errors such as label `+1` still displaying after block is destroyed, ball penetrating through blocks frequently, concurrent errors etc
    - The issues occur due to lack of synchronization in the code, multiple threads access shared data without proper synchronization
2. **Timeline Integration in `GameEngine` class**:
    - JavaFX Timeline is designed to work well with the UI thread ensures animations itself run on the UI thread, making it easier to handle animations and updates in a graphical user interface and eliminating the need for explicit synchronization in many cases.
...
1. **Add a boolean flag `leveldone`**
    ...
    - This ensures that the `nextLevel` method can only invoke one time.

### Game Ball Penetration Through Blocks
1. **Ball direction move vertically up or down**:
    - In this case, sometimes the ball straight penetrate through the blocks even the velocity of ball is slow.
2. **Ball bouncing between multiple blocks with high velocity**
    - In this case, sometimes the blocks beside hit block will be destroyed as well.
    - This is because when ball hit a blocks but didn't bounce immediately and bounce when the ball is already inside the blocks and due to the bouncing angle hit the blocks beside, seems like the ball is penetrating.
...
4. **Attempted implementation to solve the issues**
